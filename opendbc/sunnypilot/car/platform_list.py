...
CAR_LIST_WITH_EXTRAS_JSON_OUT = os.path.join(BASEDIR, "../", "sunnypilot", "car", "car_list_extra.json")
...
def get_car_list_with_extras() -> dict[str, dict[str, list[str] | str]]:
  collected_footnote = get_all_footnotes()
  sorted_list: dict[str, dict[str, list[str] | str]] = build_sorted_car_list_with_extras(PLATFORMS, collected_footnote)
  return sorted_list
...
def build_sorted_car_list_with_extras(platforms, footnotes) -> dict[str, dict[str, list[str] | str]]:
...
if __name__ == "__main__":
  ...
  platform_list_with_extras = get_car_list_with_extras()
  ...
  with open(CAR_LIST_WITH_EXTRAS_JSON_OUT, "w") as json_file:
    json.dump(platform_list_with_extras, json_file, indent=2, ensure_ascii=False)
  print(f"Generated and written to {CAR_LIST_WITH_EXTRAS_JSON_OUT}")
