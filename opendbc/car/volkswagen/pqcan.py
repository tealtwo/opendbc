...
def create_lka_hud_control(packer, bus, ldw_stock_values, lat_active, steering_pressed, hud_alert, hud_control, pulse=0):
  ...
  values.update({
    "LDW_Kameratyp": 1,
    "LDW_Lampe_gelb": 1 if pulse or steering_pressed or not lat_active else 0,
...
def create_acc_buttons_control(packer, bus, gra_stock_values, longitudinalControl, cancel=False, resume=False):
...
def acc_hud_status_value(main_switch_on, acc_faulted, gasPressed, longActive, longOverride):
  if gasPressed or longOverride and longActive:
    hud_status = 4
